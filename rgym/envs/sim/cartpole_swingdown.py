...
class Env(gym.Env):
    ...
    def __init__(self, t_limit=float('inf')):
        ...
        self.angle_cur = 0
        self.angle_delta = 0
        self.angle_threshold = (180 / 360) * (2 * math.pi)  # rad
    ...
    def step(self, action=None):
        ...
        return np.array(self.state), round(reward, 8), done, {'f': force}

    def reset(self):
        self.position_pre, self.angle_pre = 0, 0

        self.x, self.x_dot, self.theta_dot = self.np_random.uniform(low=-0.05, high=0.05, size=(3,))
        self.theta = math.pi + self.np_random.uniform(low=-0.3, high=0.3)
